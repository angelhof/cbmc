  ...
  const guardt &guard)
{
  ...
  if(expr.id()==ID_typecast)
  {
    ...
    {
      // float-to-float
      const isinf_exprt op_inf(op);
      const isinf_exprt new_inf(expr);

      or_exprt overflow_check(op_inf, not_exprt(new_inf));
  ...
  else if(expr.id()==ID_div)
  {
    ...
    const isinf_exprt new_inf(expr);
    const isinf_exprt op0_inf(to_div_expr(expr).dividend());
          ...
          expr.id()==ID_minus)
  {
    if(expr.operands().size()==2)
    {
      // Can overflow
      const isinf_exprt new_inf(expr);
      const isinf_exprt op0_inf(to_binary_expr(expr).op0());
      const isinf_exprt op1_inf(to_binary_expr(expr).op1());
